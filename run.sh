...
if [[ "$AWS_PROFILE" = "" ]]; then
    echo "error: please provide AWS_PROFILE variable"
    exit 1;
fi
if [[ "$S3_PATH" = "" ]]; then
    echo "error: please provide S3_PATH variable, i.e. s3://example-bucket/igw-logs/"
    exit 1;
fi
...
aws s3 cp --profile=$AWS_PROFILE $FN.tar.gz $S3_PATH

rm -f $FN.tar.gz
rm -rf out
aws s3 ls --profile=$AWS_PROFILE $S3_PATH
